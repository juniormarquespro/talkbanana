...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ cadastro?: string }> }) {
  ...
  const creditosBaixos = isPro ? creditos <= 50 : creditos <= 3;
      ...
      <nav className="fixed top-0 left-0 right-0 z-30" style={{
        ...
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="px-3 py-1 rounded-full text-xs font-bold" style={{
              background: isPro ? "rgba(255,193,7,0.15)" : "rgba(201,168,76,0.08)",
              border: isPro ? "1px solid rgba(255,193,7,0.4)" : "1px solid rgba(201,168,76,0.2)",
              color: isPro ? "#FFC107" : "#c9a84c",
            }}>
              {isPro ? "✦ Pro" : "Gratuito"}
            </span>
            <form action={logout}>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {creditosBaixos && creditos > 0 && (
            <div className="rounded-xl p-4 mb-5 flex items-center justify-between gap-3" style={{
              ...
              <p className="text-sm" style={{ color: "#f87171" }}>
                ⚠️ Restam apenas <strong>{creditos} crédito{creditos !== 1 ? "s" : ""}</strong>.{" "}
                {isPro ? "Renovam dia 1 do próximo mês." : "Adquira um pacote ou assine o Pro."}
              </p>
              <Link href="/precos" className="text-xs font-bold px-3 py-1.5 rounded-lg whitespace-nowrap" style={{
                background: "rgba(255,193,7,0.15)", border: "1px solid rgba(255,193,7,0.4)", color: "#FFC107",
              }}>Ver planos</Link>
            </div>
          )}
          ...
          {creditos === 0 && (
            <div className="rounded-xl p-4 mb-5 flex items-center justify-between gap-3" style={{
              ...
              <p className="text-sm font-semibold" style={{ color: "#f87171" }}>
                🚫 Créditos esgotados.{" "}
                {isPro ? "Renovam automaticamente dia 1 do próximo mês." : "Adquira um pacote ou assine o Pro."}
              </p>
              <Link href="/precos" className="text-xs font-bold px-3 py-1.5 rounded-lg whitespace-nowrap" style={{
                background: "rgba(255,193,7,0.2)", border: "1px solid rgba(255,193,7,0.6)", color: "#FFC107",
              }}>Ver planos →</Link>
            </div>
          )}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Link href="/traduzir" className="rounded-2xl p-6 flex flex-col gap-3 transition-all active:scale-[0.98]" style={{
              background: "linear-gradient(to bottom, rgba(255,193,7,0.15), rgba(255,193,7,0.03))",
              border: "1px solid rgba(255,193,7,0.3)",
              backdropFilter: "blur(10px)",
            }}>
              <div className="text-3xl">🎙️</div>
              <h3 className="font-bold text-white text-lg">Traduzir ao vivo</h3>
              <p className="text-sm" style={{ color: "rgba(201,168,76,0.7)" }}>Fale e o TalkBanana traduz em tempo real</p>
            ...
            <Link href="/precos" className="rounded-2xl p-6 flex flex-col gap-3 transition-all active:scale-[0.98]" style={{
              ...
              <p className="text-sm" style={{ color: "rgba(201,168,76,0.7)" }}>
                {isPro ? "600 créditos/mês, renovados dia 1. Obrigado!" : "600 créditos mensais, renovados automaticamente"}
              </p>
              <span className="text-xs font-semibold mt-auto self-end px-3 py-1 rounded-full" style={{
